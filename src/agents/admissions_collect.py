...
from .utils import language_instruction, latest_user_text, resolve_prompt, format_user_context
...
async def admissions_collect_node(
    ...
    # ── Stage 1: Extraction ──────────────────────────────────────────────────
    # Admissions data (student, guardian, contact...) often arrives split
    # across several rapid WhatsApp messages — extract from the whole burst.
    ...
    if last_user_msg:
        ...
        turn_usage.append(
            make_usage_record(
                node="admissions_collect.extraction", provider=provider, model=model,
            )
        )
        ...
        try:
            ...
            logger.warning("admissions_collect_extraction_parse_failed", raw=extracted_text[:200])
    ...
    turn_usage.append(
        make_usage_record(
            node="admissions_collect.reply", provider=provider, model=model,
        )
    )
